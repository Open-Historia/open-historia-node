/*! Open Historia node — auto-updater (TUF-style) © 2026 Nicholas Krol, MIT. */
// Keeps the node software current and tamper-proof. Polls a signed update
// manifest + short-lived signed timestamp, verifies both against the pinned root
// key, and applies an update ONLY if it is validly signed, for this channel,
// MONOTONICALLY newer (no rollback), at least as new as the timestamp (no
// freeze), and unexpired. Artifacts are hash-verified before staging.
//
//   OH_UPDATE_BASE_URL=https://updates.open-historia.example/stable node scripts/updater.mjs
...
const POLL_MS = Number(process.env.OH_UPDATE_POLL_MS) || 3600000;
...
export const evaluateUpdate = ({ installedVersion, updateManifest, timestamp, nowMs = Date.now() }) => {
  ...
  if (updateManifest.channel && updateManifest.channel !== CHANNEL) return { shouldUpdate: false, reason: "wrong-channel" };
  ...
  if (updateManifest.expires && Date.parse(updateManifest.expires) < nowMs) return { shouldUpdate: false, reason: "expired" };
...
const fetchVerifiedManifest = async (name) => {
  const [bytes, sig] = await Promise.all([fetchBytes(`${BASE_URL}/${name}`), fetchBytes(`${BASE_URL}/${name}.sig`).then((b) => b.toString("utf8"))]);
...
const runOnce = async () => {
  ...
  try {
    ...
    console.warn(`timestamp: ${e.message}`);
  }
  ...
  for (const artifact of updateManifest.artifacts ?? []) {
    const bytes = await fetchBytes(artifact.url);
    if (artifact.sha256 && sha256(bytes) !== artifact.sha256) throw new Error(`artifact ${artifact.path} failed hash check`);
  ...
  console.log(`Staged v${decision.version} at ${stageDir}. Swap it in and restart the node service.`);
  return true;
};
